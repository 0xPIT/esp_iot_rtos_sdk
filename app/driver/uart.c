...
void UART0_GetRxQueue(xQueueHandle *qh) {
  *qh = xQueueUart;
}
...
LOCAL void uart0_rx_intr_handler(void *para) {
    /* uart0 and uart1 intr combine togther, 
       when interrupt occur, see reg 0x3ff20020, bit2, bit0 represents
    ...
    while (uart_intr_status != 0x0) {
        if (UART_FRM_ERR_INT_ST == (uart_intr_status & UART_FRM_ERR_INT_ST)) {
            ...
            while (buf_idx < fifo_len) {
                ...
                xQueueSendFromISR(xQueueUart, (void *)&RcvChar, &xHigherPriorityTaskWoken);
                buf_idx++;
            }
...
void ICACHE_FLASH_ATTR uart_init_new(void) {
    ...
    xQueueUart = xQueueCreate(32, sizeof(uint8_t));
...
LOCAL void uart_rx_intr_handler_ssc(void) {
    /* uart0 and uart1 intr combine togther, when interrupt occur, 
        see reg 0x3ff20020:
        bit2, bit0 represents uart1 and uart0 respectively
    */
    //os_event_t e;
    portBASE_TYPE xHigherPriorityTaskWoken;
...
#if 0
void ICACHE_FLASH_ATTR uart_queue_init(void) {
    ...
    //UART_intr_handler_register(uart0_rx_intr_handler);
    UART_intr_handler_register(uart_rx_intr_handler_ssc);
...
#if 0
LOCAL void ICACHE_FLASH_ATTR uart_config(uint8 uart_no, UartDevice *uart) {
    if (uart_no == UART1) {
        PIN_FUNC_SELECT(PERIPHS_IO_MUX_GPIO2_U, FUNC_U1TXD_BK);
    } else {
